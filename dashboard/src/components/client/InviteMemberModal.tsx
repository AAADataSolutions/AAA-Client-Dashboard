  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#15161c] border border-slate-200 dark:border-[#222430] rounded-2xl shadow-2xl p-6 z-10 space-y-5 animate-in zoom-in-95 duration-200">
        ...
        {generatedInviteLink ? (
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/40 space-y-3 animate-in fade-in">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-2.5 py-1.5 bg-white dark:bg-[#111217] border border-blue-200 dark:border-blue-900/60 rounded-lg text-[11px] text-slate-800 dark:text-slate-200 select-all outline-hidden"
              />
              <button
                onClick={copyToClipboard}
                className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs flex items-center gap-1 shrink-0 transition cursor-pointer"
              >
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-300" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copied ? 'Copied' : 'Copy'}</span>
              </button>
            </div>
            <div className="pt-2 border-t border-blue-100 dark:border-blue-900/30 flex justify-end">
              <button
                type="button"
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div
                  ...
                  className={`p-3 rounded-xl border cursor-pointer transition ${
                    targetOrgRole === 'USER'
                      ? 'bg-blue-50/50 dark:bg-blue-950/40 border-blue-500 text-slate-900 dark:text-white'
                ...
                >
                  <div className="font-semibold text-xs flex items-center gap-1">
                    <Shield className="w-3 h-3 text-blue-500" /> Client Admin
                  </div>
                  <p className="text-[10.5px] text-slate-500 dark:text-slate-400 mt-0.5">
